    ...
    F: Fn(&T, &T) -> T,
{
    ...
    fn query_inner(
        ...
        i: usize,
        inner_range: impl RangeBounds<usize>,
    ) -> T {
        ...
        let left = self.query_inner(
            range.start..mid.min(range.end),
            i << 1,
            inner_range.start..mid,
        );
        let right = self.query_inner(
            mid.max(range.start)..range.end,
            (i << 1) + 1,
            mid..inner_range.end,
        );
        (self.f)(&left, &right)
...
impl<T, F, G, H> SegmentTreeRU<T, F, G, H>
where
    T: Clone + Copy,
    F: Fn(&T, &T) -> T,
    G: Fn(&T, &T, usize) -> T,
    H: Fn(&T, &T) -> T,
{
    ...
    pub fn new_from_iter<I>(iter: I, neutral: T, f: F, g: G, h: H) -> Self
    where
        I: IntoIterator<Item = T>,
    {
        let v: Vec<_> = iter.into_iter().collect();
